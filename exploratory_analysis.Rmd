...
anova(interaction_model2, final_model)
```
...
library(ggResidpanel)
pop_resids <- ggplot(aug_final, aes(x = log(pop_15to64), y = .resid)) + geom_point() + labs(x = "log(Population)", y = "Residuals", title = "tbd")
urb_resids <- ggplot(aug_final, aes(x = urbanicity, y = .resid)) + geom_boxplot() + labs(x = "Urbanicity", y = "Residuals", title = "tbd")
race_resids <- ggplot(aug_final, aes(x = race, y = .resid)) + geom_boxplot() + labs(x = "Race", y = "Residuals", title = "tbd")
pop_resids / (urb_resids + race_resids)
resid_xpanel(final_model)
resid_panel(final_model, plots = c("qq", "resid"))

```

```{r}
#final model
ggplot(aug_final, aes(x = log(pop_15to64), y = .fitted)) + geom_point()
