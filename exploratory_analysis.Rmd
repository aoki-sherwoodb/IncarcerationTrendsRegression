...
```{r}
#final model
pop_plot <- ggplot(aug_final, aes(x = log(pop_15to64), y = log1p(jail_pop))) + geom_point() + geom_smooth(se = FALSE, method = "lm") + facet_wrap(vars(race)) + labs(x = "Log(Population)", y = "Log(Jail Population)")
#urb_plot <- ggplot(aug_final, aes(x = urbanicity, y = log1p(jail_pop))) + geom_boxplot() + labs(x = "Urbanicity", y = "Log(Jail Popuation") + facet_wrap(vars(race))
urb_plot <- ggplot(aug_final, aes(x = urbanicity, y = log(pop_15to64))) + geom_boxplot() + labs(x = "Urbanicity", y = "Log(Jail Popuation") + facet_wrap(vars(race))
urb_plot
```
