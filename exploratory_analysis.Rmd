...
joined_data_no_na %>% 
  ggplot(aes(x = log(pop_15to64), y = log(jail_pop_rate), color = race)) +
    geom_point() +
...
```{r}
null_model <- lm(log1p(jail_pop_rate) ~ log(pop_15to64) + urbanicity + median_household_income)
full_model <- lm(log1p(jail_pop_rate) ~ log(pop_15to64)*race + urbanicity + log(median_household_income) + gini_index)
```
