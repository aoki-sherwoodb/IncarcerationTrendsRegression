...
long_incarc <- tx_incarc %>%
  ...
  filter(!(total.jail_pop_rate != 0.0 & aapi.jail_pop_rate == 0.0 & 
             black.jail_pop_rate == 0.0 & latinx.jail_pop_rate == 0.0 & 
             native.jail_pop_rate == 0.0 & white.jail_pop_rate == 0.0)) %>%
...
joined_data <- left_join(long_acs, long_incarc, by = c("county_name", "race")) %>%
  drop_na() %>%
  mutate(urban_category = ifelse(urbanicity == "rural" | urbanicity == "small/mid", "rural", "urban"))
...
```{r}
# check relationship of controls (population, urbanicity) with jail rate (including 0s!)
...
joined_data %>%
  filter(race == "all") %>%
  ggplot(aes(x = log(pop_15to64), y = log1p(jail_pop_rate))) +
    geom_point()
```
...
joined_data %>% 
  ggplot(aes(x = log(pop_15to64), y = log1p(jail_pop_rate), color = race)) +
    geom_point() +
...
```{r}
joined_data %>%
  ggplot(aes(x = log(median_household_income), y = log1p(jail_pop_rate))) +
  geom_point() +
  geom_smooth(method = "lm", se = F) +
  facet_grid(cols = vars(urban_category))
```
...
```{r}
null_model <- lm(log1p(jail_pop_rate) ~ race + log(pop_15to64), data = joined_data)
aug_null <- augment(null_model, newdata = joined_data)
pop_inc_model <- lm(log1p(jail_pop_rate) ~ race + log(pop_15to64) + log(median_household_income), data = joined_data)
aug_pop_inc <- augment(pop_inc_model, newdata = joined_data)
full_model <- lm(log1p(jail_pop_rate) ~ log(pop_15to64) + log(median_household_income) + urbanicity + race, data = joined_data)
aug_full <- augment(full_model, newdata = joined_data)
```
...
```{r}
joined_data %>%
  filter(race == "native" & jail_pop_rate == 0.0) %>%
  count()
```
